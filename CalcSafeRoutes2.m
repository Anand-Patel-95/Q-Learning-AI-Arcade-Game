function safe_routes = CalcSafeRoutes2(G, explorer, enemy1, enemy2, enemy3, checkIntersections, IntersectionTable, NodeTable)
% make sure explorer location is not counted as intersection
IntersectionTable = IntersectionTable(IntersectionTable.ID ~= explorer.id,:);
...
for i=1:1:length(Intersection_List)
    [temp_path, Intersection_List(i,2)] = shortestpath(G,explorer.id, Intersection_List(i,1));
